...
def upgrade() -> None:
    ...
    op.execute(
        "INSERT INTO environments (id, code, name, description, active) "
        "SELECT '55555555-5555-5555-5555-555555555555', 'REGRESI', 'Regresi Environment', 'Regression testing environment', 1 "
        "WHERE NOT EXISTS (SELECT 1 FROM environments WHERE code = 'REGRESI')"
    )

    # 3. Insert STAGING environment if it doesn't exist
    op.execute(
        "INSERT INTO environments (id, code, name, description, active) "
        "SELECT '66666666-6666-6666-6666-666666666666', 'STAGING', 'Staging Environment', 'Staging pre-production testing environment', 1 "
        "WHERE NOT EXISTS (SELECT 1 FROM environments WHERE code = 'STAGING')"
    )
